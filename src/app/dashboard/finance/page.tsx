...
export default function FinancePage() {
  ...
  const handleCloture = async () => {
    ...
    const { data: newCycle } = await supabase.from('finance_cycles').insert({
      shop_id: SHOP_ID, start_date: new Date().toISOString().split('T')[0], status: 'active'
    }).select().single()
  ...
  const loadVentesForCycle = async (cycle: Cycle) => {
    const startISO = new Date(cycle.start_date).toISOString()
    const endISO = cycle.end_date ? new Date(cycle.end_date + 'T23:59:59').toISOString() : new Date().toISOString()
    const { data: boutique } = await supabase.from('sales').select('*, sale_items(*), customer:customers(full_name)').eq('shop_id', SHOP_ID).eq('status', 'paid').gte('created_at', startISO).lte('created_at', endISO)
    const { data: site } = await supabase.from('orders').select('*').eq('shop_id', SHOP_ID).eq('status', 'livré').gte('created_at', startISO).lte('created_at', endISO)
  ...
  const handleGeneratePdf = async (cycle: Cycle, ventes: any[]) => {
    ...
    try {
      ...
      const cGold: [number, number, number] = [212, 168, 67]
      const cBlack: [number, number, number] = [26, 26, 46]
      const cGray: [number, number, number] = [150, 150, 150]
      const cLight: [number, number, number] = [245, 245, 245]
      const cGreen: [number, number, number] = [16, 185, 129]
      const cBlue: [number, number, number] = [59, 130, 246]
      const cMid: [number, number, number] = [100, 100, 100]
      ...
      doc.setTextColor(cGold[0], cGold[1], cGold[2])
      doc.setFont('helvetica', 'bold')
      doc.text('RAPPORT FINANCIER', 190, 18, { align: 'right' })
      doc.setFontSize(9)
      doc.setTextColor(180, 180, 180)
      doc.setFont('helvetica', 'normal')
      doc.text('Cycle : ' + fmtDate(cycle.start_date) + ' — ' + fmtDate(cycle.end_date || ''), 190, 26, { align: 'right' })
      doc.text('Genere le ' + new Date().toLocaleDateString('fr-FR'), 190, 32, { align: 'right' })
      ...
      const stats = [
        { label: 'CA Brut', value: fmt(cycle.ca_brut) + ' FCFA', bold: false },
        { label: 'CA Net', value: fmt(cycle.ca_net) + ' FCFA', bold: false },
        { label: 'Cout de revient', value: fmt(cycle.cout_revient) + ' FCFA', bold: false },
        { label: 'Marge Brute', value: fmt(cycle.marge_brute) + ' FCFA', bold: false },
        { label: 'Charges Fixes', value: fmt(cycle.charges_fixes_total) + ' FCFA', bold: false },
        { label: 'Marge Nette', value: fmt(cycle.benefice_net) + ' FCFA', bold: true },
      ]
      ...
      stats.forEach((stat, i) => {
        ...
        doc.setFont('helvetica', 'bold')
        doc.text(stat.value, 188, y + 2, { align: 'right' })
        y += 10
      })
      ...
      // REPARTITION
      if (cycle.benefice_net > 0 && repartition.length > 0) {
      ...
      if (ventes.length > 0) {
        ...
        ventes.forEach((v, i) => {
          if (y > 270) { doc.addPage(); y = 20; drawTableHeader() }
          ...
          if (v.source === 'site') {
            ...
            doc.setTextColor(cMid[0], cMid[1], cMid[2])
          }
      ...
      doc.save('CJS-rapport-' + cycle.start_date + '-' + (cycle.end_date || 'actuel') + '.pdf')
  ...
  return (
    <div style={{ fontFamily: "'Sora', sans-serif", background: '#f8f8f8', minHeight: '100vh', color: '#1a1a2e' }}>
      ...
      {/* TOPBAR */}
      <div style={{ background: 'white', borderBottom: '1px solid #eee', padding: '18px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 50, flexWrap: 'wrap', gap: '12px' }}>
        <div>
      ...
      <div style={{ padding: '24px', maxWidth: '1100px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div className="section-card">
          <div className="section-header">
            <div style={{ fontSize: '14px', fontWeight: 600 }}>Charges Fixes</div>
            <button className="btn-ghost" onClick={() => setShowAddCharge(!showAddCharge)}>+ Ajouter</button>
          </div>
        ...
        <div className="section-card">
          <div className="section-header">
            <div style={{ fontSize: '14px', fontWeight: 600 }}>Repartition Automatique</div>
            <button className="btn-ghost" onClick={() => setShowAddRep(!showAddRep)}>+ Ajouter</button>
          </div>
          <div className="section-body">
            ...
            {showAddRep && (
              <div style={{ marginTop: '14px', background: '#fafafa', padding: '14px', borderRadius: '8px', border: '1px dashed #ddd', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                <input className="form-input" style={{ flex: 1, minWidth: '140px' }} placeholder="Nom (ex: Epargne...)" value={repForm.name} onChange={e => setRepForm({ ...repForm, name: e.target.value })} />
                  ...
                  <div key={cycle.id} className="cycle-row">
                    ...
                    <div style={{ display: 'flex', gap: '8px', flexShrink: 0, flexWrap: 'wrap' }}>
                      ...
                      <button className="btn-ghost" disabled={generatingPdf} onClick={async () => {
                        const ventes = await loadVentesForCycle(cycle)
                        await handleGeneratePdf(cycle, ventes)
                      }}>
      ...
      {showClotureModal && (
        <div className="modal-overlay" onClick={() => setShowClotureModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ background: '#fff0f0', border: '1px solid #fca5a5', borderRadius: '8px', padding: '12px 14px', fontSize: '12px', color: '#e53e3e', marginBottom: '16px' }}>
              Action irreversible. Verifiez que toutes les ventes sont enregistrees.
            </div>
            ...
            <div style={{ marginBottom: '16px' }}>
              <label style={{ fontSize: '12px', color: '#888', display: 'block', marginBottom: '6px' }}>Tapez CONFIRMER pour valider :</label>
