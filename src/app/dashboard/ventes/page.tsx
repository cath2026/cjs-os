...
import { Plus, ShoppingCart, Globe, Search, ChevronRight } from 'lucide-react'
...
const STATUS_ORDER = ['commandé', 'en preparation', 'en livraison', 'livré']

const getNextStatuses = (currentStatus: string): string[] => {
  const idx = STATUS_ORDER.indexOf(currentStatus)
  if (idx === -1 || idx === STATUS_ORDER.length - 1) return []
  // Un seul bouton : le statut suivant uniquement
  // + bouton annuler séparé seulement si pas encore livré
  const next = [STATUS_ORDER[idx + 1]]
  if (STATUS_ORDER[idx + 1] !== 'livré') next.push('annulé')
  return next
}
...
export default function VentesPage() {
  ...
  const tabs = [
    { key: 'all', label: 'Toutes' }, { key: 'draft', label: 'Brouillon' },
    { key: 'in_delivery', label: 'Livraison' }, { key: 'paid', label: 'Payées' },
  ...
  const filteredSales = sales
    ...
    .filter(s => {
      ...
      return name.includes(q) || formatDate(s.created_at).includes(q) || s.total.toString().includes(q)
    })
  ...
  const filteredOrders = orders.filter(o => {
    ...
    return (o.customer_name || '').toLowerCase().includes(q) ||
      ...
      formatDate(o.created_at).includes(q) ||
  ...
  const updateOrderStatus = async (orderId: string, newStatus: string, order: Order) => {
    ...
    if (newStatus === 'livré') {
      await supabase.from('sales').insert({
        shop_id: SHOP_ID, total: order.total, discount_amount: order.discount || 0,
        status: 'paid', customer_name: order.customer_name || 'Client site',
        acquisition_source: 'site', notes: `Commande site #${order.order_ref}`,
      })
    }
    await fetchOrders()
    await fetchSales()
  ...
  const getItemBarcode = (item: any): string => {
    if (item.isCustom) return 'CJS-CUSTOM'
    if (item.barcode && typeof item.barcode === 'string' && item.barcode.startsWith('CJS-')) return item.barcode
    return '—'
  }
    ...
    <div className="p-4 lg:p-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-stone-500 text-xs">{sales.length} boutique · {orders.length} site</p>
        </div>
        {mainTab === 'boutique' && (
          <button onClick={() => router.push('/dashboard/ventes/nouvelle')}
      ...
      <div className="relative mb-3">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)}
          placeholder={mainTab === 'boutique' ? 'Client, date, montant...' : 'Client, n° commande, date...'}
      ...
      <div className="flex gap-2 mb-4">
        <button onClick={() => setMainTab('boutique')}
          ...
          <ShoppingCart size={13} /> Boutique ({sales.length})
        </button>
        <button onClick={() => setMainTab('site')}
          ...
          <Globe size={13} /> Site ({orders.length})
          ...
          <div className="flex gap-1.5 mb-3 flex-wrap">
            {tabs.map(tab => (
              <button key={tab.key} onClick={() => setFilter(tab.key)}
                className={`px-3 py-1 rounded-full text-xs font-medium ${filter === tab.key ? 'bg-yellow-600 text-white' : 'bg-white text-stone-600 border border-stone-200'}`}>
                {tab.label}
              </button>
            ))}
          </div>
                ...
                <div key={sale.id} onClick={() => router.push(`/dashboard/ventes/${sale.id}`)}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right flex-shrink-0 ml-2">
                      ...
                      {sale.discount_amount > 0 && <p className="text-xs text-red-400">-{formatFCFA(sale.discount_amount)}</p>}
                    </div>
                  </div>
                </div>
              ))}
          ...
          {filteredOrders.length === 0 ? (
            ...
            <div className="space-y-3">
              {filteredOrders.map(order => {
                ...
                const isTerminal = order.status === 'livré' || order.status === 'annulé'
                ...
                return (
                  <div key={order.id} className="bg-white rounded-xl p-4 shadow-sm">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {order.customer_phone && (
                          <a href={`https://wa.me/${order.customer_phone.replace(/\D/g, '')}`}
                            target="_blank" rel="noreferrer" className="text-xs text-green-600 hover:underline">
                            📱 {order.customer_phone}
                          </a>
                        )}
                      ...
                      {order.items?.map((item: any, i: number) => (
                        <div key={i} className="flex justify-between items-start text-xs py-1.5 border-b border-stone-100 last:border-0">
                          <div className="min-w-0 flex-1 mr-3">
                            ...
                            <p className="text-stone-400 font-mono">{getItemBarcode(item)}</p>
                    ...
                    {/* Statut — progression linéaire */}
                    {isTerminal ? (
                      <p className={`text-xs font-medium ${order.status === 'livré' ? 'text-green-600' : 'text-red-400'}`}>
                        {order.status === 'livré' ? '✓ Livrée · CA enregistré' : '✕ Annulée'}
                      </p>
                    ) : (
                      <div>
                        <p className="text-xs text-stone-400 mb-1.5 flex items-center gap-1">
                          <ChevronRight size={11} /> Avancer la commande :
                        </p>
                        <div className="flex gap-1.5 flex-wrap">
                          {nextStatuses.map(status => (
                            <button key={status} disabled={isUpdating}
                              ...
                              {isUpdating ? '...' : status === 'livré' ? '✓ Marquer livré' : `→ ${status}`}
                            </button>
                          ))}
                        </div>
                      </div>
